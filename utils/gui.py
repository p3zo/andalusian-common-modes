"""Adapted from https://github.com/MTG/andalusian-corpus-notebooks/blob/master/gui/gui_metadata.py"""
...
class VisualizeDataframeGui:
    def __init__(self, MetadataObject):
        ...
        for characteristic in DF_LISTS[1:5]:
            ...
            for index in df.index.values.tolist():
                ...
                if len(list_value) != 0:
                    ...
                    transliterated_names.append(
                        widgets.Label(str(df.loc[index, COLUMNS_NAMES[1]]))
                    )
                    ...
                    num_sections.append(
                        widgets.Label(str(df_sections.loc[index, "num_sections"]))
                    )

                    # overall amount of time
                    times_values.append(df_times.loc[index, "time"])
                    times.append(
                        widgets.Label(str(get_time(df_times.loc[index, "time"])))
                    )

                    # avarage amount of time
                    avg_times_values.append(df_avg.loc[index, "avg"])
                    avg_times.append(
                        widgets.Label(str(get_time(df_avg.loc[index, "avg"])))
                    )

            id_layout = widgets.Layout(
                display="flex",
                flex_flow="column",
                align_items="stretch",  # border='solid 1px',\
                width="6%",
            )

            tran_layout = widgets.Layout(
                display="flex",
                flex_flow="column",
                align_items="stretch",  # border='solid 1px',\
                width="21%",
        ...
        figure_plot = widgets.interactive(
            self.plot_bar_histogram, type=self.type_selector, tab=self.tab_selector
        )
    ...
    def plot_bar_histogram(
        self, type, tab
    ):  # NB: tab is the widget, not the musical entity
        ...
        if type == self.column_titles[0]:
            ...
            if type == self.column_titles[1]:
                ...
                if type == self.column_titles[2]:
                    ...
                    plt.yticks(
                        np.arange(0, max(self.times_values[tab_index]), 10800),
                        (
                            np.arange(0, max(self.times_values[tab_index]), 10800)
                            / 3600
                        ).astype(int),
                    ...
                    if type == self.column_titles[3]:
                        ...
                        plt.yticks(
                            np.arange(0, max(self.avg_times_values[tab_index]), 120),
                            (
                                np.arange(0, max(self.avg_times_values[tab_index]), 120)
                                / 60
                            ).astype(int),
...
class CrossMetadataVisualization:
    def __init__(self, MetadataObject):
        ...
        style_dropdown = widgets.Layout(
            width="180px"
        )  # TODO: fix the width using percentage
        for i in range(4):
            label = widgets.Label(labels[i])
            if i < 2:
                self.dropbox_menus.append(
                    widgets.Dropdown(
                        options=DF_LISTS[1:5],
                        value=DF_LISTS[i + 1],
                        layout=style_dropdown,
                    )
                )
            else:
                if i == 2:
                    ...
                    self.dropbox_menus.append(
                        widgets.Dropdown(
                            options=self.options_title,
                            value=self.options_title[0],
                            layout=style_dropdown,
                        )
                    )
            top_boxes.append(
                widgets.VBox([label, self.dropbox_menus[i]], style=style_boxdropdown)
            )
    ...
    def updateResults(self, change):
        ...
        if title_type != "id":
            for id in values_first_column:
                translated_id_values.append(
                    self.cm.convert_id(id, row_type, title_type)
                )
        else:
        ...
        if title_type != "id":
            for id in values_other_columns:
                translated_other_id_values.append(
                    self.cm.convert_id(id, column_type, title_type)
                )
        else:
        ...
        for col in list(df_results.columns.values):
            ...
            for element in df_results.index.values.tolist():
                ...
                if (
                    df_results.loc[element, col] == get_time(0)
                    or df_results.loc[element, col] == 0
                ):
                    single_column.append(
                        widgets.Label(str(df_results.loc[element, col]))
                    )
                else:
                    layout_box = widgets.Layout(border="solid 1px red")
                    single_column.append(
                        widgets.Box(
                            children=[widgets.Label(str(df_results.loc[element, col]))],
                            layout=layout_box,
                        )
                    )
        ...
        self.third_line.children = [
            widgets.HBox(children=all_columns)
        ]  # , layout = layout_hbox)]
