...
from .generalUtilities import get_interval, get_seconds, get_time, list_intersection
...
class CollectionMetadata:
    def __init__(self):
        ...
        for i in range(1, 5):
            ...
            with open(
                os.path.join(DATA_DIR, PREFIX_JSON + DF_LISTS[i] + ".json")
            ) as json_file:
        ...
        for row in descriptions_file:
            ...
            self.df_recording.at[
                row[COLUMNS_DESCRIPTION[0]], COLUMNS_RECORDINGS[2]
            ] = row[COLUMNS_RECORDINGS[2]]
            self.df_recording.at[
                row[COLUMNS_DESCRIPTION[0]], COLUMNS_RECORDINGS[3]
            ] = row[COLUMNS_RECORDINGS[3]]
            ...
            for section in row["sections"]:
                ...
                self.df_description.loc[counter] = [
                    mbid,
                    counter_sec,
                    t,
                    n,
    ...
    def search_recording(
        self, id_tab="all", id_nawba="all", id_mizan="all", id_form="all"
    ):
        ...
        if id_tab in self.df_tab.index.tolist():
            mbid_list = list_intersection(
                mbid_list, self.search_recordings_by_type("tab", id_tab)
            )

        # search for nawba
        if id_nawba in self.df_nawba.index.tolist():
            mbid_list = list_intersection(
                mbid_list, self.search_recordings_by_type("nawba", id_nawba)
            )

        # search for mizan
        if id_mizan in self.df_mizan.index.tolist():
            mbid_list = list_intersection(
                mbid_list, self.search_recordings_by_type("mizan", id_mizan)
            )

        # search for form
        if id_form in self.df_form.index.tolist():
            mbid_list = list_intersection(
                mbid_list, self.search_recordings_by_type("form", id_form)
            )
    ...
    def get_characteristic(self, rmbid, characteristic):
        ...
        if len(set(tab_list)) == 0:
            raise Exception(
                "rmbid {} has not {} description".format(rmbid, characteristic)
            )
        if len(set(tab_list)) > 1:
            raise Exception(
                "rmbid {} has more than one {} in the same recording".format(
                    rmbid, characteristic
                )
            )
        return tab_list[0]
    ...
    def get_avarage_sections_time(self, type_dataframe):
        ...
        for index in df_times.index.values.tolist():
            if df_times.loc[index, "time"] == 0:
                ...
                df_avg.loc[index, "avg"] = int(
                    df_times.loc[index, "time"]
                    / df_num_sections.loc[index, "num_sections"]
                )
    ...
    def get_recording_orchestra_name(self, rmbid):
        return str(
            "("
            + self.df_recording.loc[rmbid, "archive_url"].split("/")[-1].split("_")[0]
            + ")"
        )
    ...
    def get_cross_dataframe(self, column_type, row_type, statistic_type):
        ...
        """
        if not (
            column_type in DF_LISTS[1:5]
            and row_type in DF_LISTS[1:5]
            and statistic_type in STATISTIC_TYPE
        ):
        ...
        df_cross = pd.DataFrame(
            0, index=df_index.index.tolist(), columns=df_column.index.tolist()
        )
        df_cross_temp = pd.DataFrame(
            0, index=df_index.index.tolist(), columns=df_column.index.tolist()
        )
        mbid_temp = ""
        for i in self.df_description.index.tolist():
    ...
    def convert_dataframe_second_to_time(self, df_seconds):
        df_time = pd.DataFrame(
            0, index=df_seconds.index.tolist(), columns=df_seconds.columns.values
        )
